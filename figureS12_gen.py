...
# Read in workplace contact matrices split on intervention stringency 
comix_work_weakest_in = {}

with open('Output/UK case study/comix_work_weakest.csv', newline='\n') as csvfile:
    reader = csv.reader(csvfile, delimiter='\t', quotechar='"')
    reader.__next__()
    for row in reader:
        comix_work_weakest_in[row[0]] = [float(h) for h in row[1:]]

comix_work_weakest = pd.DataFrame(comix_work_weakest_in, index=list(comix_work_weakest_in.keys()))

comix_work_weak_in = {}

with open('Output/UK case study/comix_work_weak.csv', newline='\n') as csvfile:
    reader = csv.reader(csvfile, delimiter='\t', quotechar='"')
    reader.__next__()
    for row in reader:
        comix_work_weak_in[row[0]] = [float(h) for h in row[1:]]

comix_work_weak = pd.DataFrame(comix_work_weak_in, index=list(comix_work_weak_in.keys()))

comix_work_strong_in = {}

with open('Output/UK case study/comix_work_strong.csv', newline='\n') as csvfile:
    reader = csv.reader(csvfile, delimiter='\t', quotechar='"')
    reader.__next__()
    for row in reader:
        comix_work_strong_in[row[0]] = [float(h) for h in row[1:]]

comix_work_strong = pd.DataFrame(comix_work_strong_in, index=list(comix_work_strong_in.keys()))


comix_work_strongest_in = {}

with open('Output/UK case study/comix_work_strongest.csv', newline='\n') as csvfile:
    reader = csv.reader(csvfile, delimiter='\t', quotechar='"')
    reader.__next__()
    for row in reader:
        comix_work_strongest_in[row[0]] = [float(h) for h in row[1:]]

comix_work_strongest = pd.DataFrame(comix_work_strongest_in, index=list(comix_work_strongest_in.keys()))
...
matrix_max = max([h for j in range(len(comix_work_weakest.values)) for h in comix_work_weakest.values[j]] + 
               [h for j in range(len(comix_work_weak.values)) for h in comix_work_weak.values[j]] +
               [h for j in range(len(comix_work_strong.values)) for h in comix_work_strong.values[j]] + 
               [h for j in range(len(comix_work_strongest.values)) for h in comix_work_strongest.values[j]])
...
sns.heatmap(comix_work_weakest, cmap="YlGnBu", cbar_kws={"format": formatter}, vmin=0, vmax = matrix_max, ax=axs[0][0]).invert_yaxis()
...
sns.heatmap(comix_work_strong, cmap="YlGnBu", cbar_kws={"format": formatter}, vmin=0, vmax = matrix_max, ax=axs[1][0]).invert_yaxis()
...
sns.heatmap(comix_work_weak, cmap="YlGnBu", cbar_kws={"format": formatter}, vmin=0, vmax = matrix_max, ax=axs[0][1]).invert_yaxis()
...
sns.heatmap(comix_work_strongest, cmap="YlGnBu", cbar_kws={"format": formatter}, vmin=0, vmax = matrix_max, ax=axs[1][1]).invert_yaxis()
...
plt.savefig('Supplement figure stringency work.pdf')